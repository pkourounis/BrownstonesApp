...
import { PostCard, type Comment, type Original, type Media } from './post-card';
...
export default async function FeedPage({
  ...
  // Category / acknowledgment filter.
  if (filter === 'ack') posts = posts.filter((p) => p.requires_ack);
  else if (filter !== 'all') posts = posts.filter((p) => p.category === filter);
  ...
  if (origIds.length) {
    const { data: origPosts } = await supabase
      ...
      .in('id', origIds);
    const { data: origAtt } = await supabase.from('post_attachments').select('post_id, url, mime').in('post_id', origIds);
    const photosByOrig = new Map<string, Media[]>();
    for (const a of origAtt ?? []) photosByOrig.set(a.post_id, [...(photosByOrig.get(a.post_id) ?? []), { url: a.url, mime: a.mime ?? '' }]);
  ...
  for (const a of attachData ?? []) photosByPost.set(a.post_id, [...(photosByPost.get(a.post_id) ?? []), { url: a.url, mime: (a as { mime?: string }).mime ?? '' }]);
    ...
    <div className="space-y-4">
      ...
      {posts.length === 0 ? (
        <div className="card text-center text-sm text-brand-500">
          {filter === 'all' ? 'Nothing posted yet.' : 'Nothing here yet.'}
        </div>
      ) : (
